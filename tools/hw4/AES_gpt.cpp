
// 这个版本似不正确
#include <iostream>
#include <vector>
#include <string>
#include <iomanip>
#include <sstream>

const unsigned char sbox[256] = {
    
};

const unsigned char rcon[10] = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

unsigned char gf_mul(unsigned char a, unsigned char b) {
    unsigned char p = 0;
    unsigned char hi_bit_set;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        hi_bit_set = a & 0x80;
        a <<= 1;
        if (hi_bit_set) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

void RotWord(unsigned char* word) {
    unsigned char temp = word[0];
    word[0] = word[1];
    word[1] = word[2];
    word[2] = word[3];
    word[3] = temp;
}

void SubWord(unsigned char* word) {
    for (int i = 0; i < 4; i++) {
        word[i] = sbox[word[i]];
    }
}

std::vector<unsigned char> KeyExpansion(const std::vector<unsigned char>& key) {
    std::vector<unsigned char> w(176);
    for (int i = 0; i < 16; i++) {
        w[i] = key[i];
    }
    unsigned char temp[4];
    for (int i = 4; i < 44; i++) {
        for (int j = 0; j < 4; j++) {
            temp[j] = w[(i-1)*4 + j];
        }
        if (i % 4 == 0) {
            RotWord(temp);
            SubWord(temp);
            temp[0] ^= rcon[i/4 - 1];
        }
        for (int j = 0; j < 4; j++) {
            w[i*4 + j] = w[(i-4)*4 + j] ^ temp[j];
        }
    }
    return w;
}

void SubBytes(std::vector<unsigned char>& state) {
    for (int i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }
}

void ShiftRows(std::vector<unsigned char>& state) {
    unsigned char temp;
    temp = state[4];
    state[4] = state[5];
    state[5] = state[6];
    state[6] = state[7];
    state[7] = temp;

    temp = state[8];
    state[8] = state[10];
    state[10] = temp;
    temp = state[9];
    state[9] = state[11];
    state[11] = temp;

    temp = state[15];
    state[15] = state[14];
    state[14] = state[13];
    state[13] = state[12];
    state[12] = temp;
}

void MixColumns(std::vector<unsigned char>& state) {
    for (int i = 0; i < 4; i++) {
        unsigned char s0 = state[i*4];
        unsigned char s1 = state[i*4 + 1];
        unsigned char s2 = state[i*4 + 2];
        unsigned char s3 = state[i*4 + 3];
        state[i*4] = gf_mul(2, s0) ^ gf_mul(3, s1) ^ s2 ^ s3;
        state[i*4 + 1] = s0 ^ gf_mul(2, s1) ^ gf_mul(3, s2) ^ s3;
        state[i*4 + 2] = s0 ^ s1 ^ gf_mul(2, s2) ^ gf_mul(3, s3);
        state[i*4 + 3] = gf_mul(3, s0) ^ s1 ^ s2 ^ gf_mul(2, s3);
    }
}

void AddRoundKey(std::vector<unsigned char>& state, const std::vector<unsigned char>& round_key, int round) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= round_key[round*16 + i];
    }
}

std::vector<unsigned char> Encrypt(const std::vector<unsigned char>& plaintext, const std::vector<unsigned char>& key) {
    std::vector<unsigned char> state = plaintext;
    std::vector<unsigned char> round_keys = KeyExpansion(key);
    AddRoundKey(state, round_keys, 0);
    for (int round = 1; round < 10; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, round_keys, round);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, round_keys, 10);
    return state;
}

int main() {
    std::string key_hex, plaintext_hex;
    std::cin >> key_hex >> plaintext_hex;
    std::vector<unsigned char> key;
    for (size_t i = 0; i < key_hex.length(); i += 2) {
        std::string byteString = key_hex.substr(i, 2);
        unsigned char byte = static_cast<unsigned char>(std::stoi(byteString, nullptr, 16));
        key.push_back(byte);
    }
    std::vector<unsigned char> plaintext;
    for (size_t i = 0; i < plaintext_hex.length(); i += 2) {
        std::string byteString = plaintext_hex.substr(i, 2);
        unsigned char byte = static_cast<unsigned char>(std::stoi(byteString, nullptr, 16));
        plaintext.push_back(byte);
    }
    std::vector<unsigned char> ciphertext = Encrypt(plaintext, key);
    std::string hex_str;
    for (unsigned char byte : ciphertext) {
        std::stringstream ss;
        ss << std::hex << std::uppercase << std::setw(2) << std::setfill('0') << static_cast<int>(byte);
        hex_str += ss.str();
    }
    std::cout << hex_str << std::endl;
    return 0;
}